        ...
        {items.map((item, idx) =>
          <li key={item.id}>
            <Link href={recordToSlug(items[idx]).split('/').slice(0, -1).join('/')}>
              ...
              <figcaption>
                {item.__typename === 'ParticipantRecord' ? t('participants') : item.__typename === 'PartnerRecord' ? t('partners') : item.__typename === 'ProgramRecord' ? t('program') : item.__typename === 'ExhibitionRecord' ? t('exhibitions') : item.__typename === 'AboutRecord' ? t('about') : null}
              </figcaption>
            </Link>
          </li>
        )}
